...
public class GridDisplayManager : ScriptableObject {
    ...
    private List<SpriteRenderer> displayedIcons = new List<SpriteRenderer>(); //displayed unitIcons
    ...
    //called in MapManager to display danger area and icons
    public void DisplayGridDetails(UnitList unitList) {
        DisplayGroupIcons(unitList);
        if (unitList.team != grid.manager.playerTeam)
            DisplayAllDanger(unitList);
    }

    //displays group icon above units, is really just an instance of the tile "valid tile" prefab
    private void DisplayGroupIcons(UnitList unitList) {
        foreach (GameObject obj in unitList.mapUnits) {
            Unit unit = obj.GetComponent<Unit>();
            SpriteRenderer icon = Instantiate(tilePrefab, unit.gridPos + new Vector2(0,1.75f), Quaternion.identity, obj.GetComponent<Transform>());
            icon.sprite = unit.groupIcon;
            icon.sortingLayerName = "Default";
    ...
    private void DisplayAllDanger(UnitList unitList) {
        allDanger = true;
        foreach (GameObject obj in unitList.mapUnits) {
    ...
    //stops displaying all danger tiles and group icons
    public void StopAllDisplay() {
        foreach (SpriteRenderer icon in displayedIcons) {
            Destroy(icon);
        }
        displayedIcons.Clear();
        StopDisplayAllDanger();
    }


    //stops allDanger danger tiles
    private void StopDisplayAllDanger() {
