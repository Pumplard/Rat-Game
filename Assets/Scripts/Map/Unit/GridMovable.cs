...
public class GridMovable : MonoBehaviour
{
    ...
    //starts idle animation at random frame
    //referenced in unitList.onTurnStart() & unitList.end();
    public void StartIdleAnimationR() {
        float r = Random.Range(0.0f,1.0f);
        animator.Play("unit_idle",0,r);
    }

    //referenced and Act() below
    //also on deselect (unitSelect)
    public void StartIdleAnimation() {
        animator.Play("unit_idle");
    }


    //called in onSelect and when move is cancelled (selectManager)
    public void MoveAnimation() {
        animator.Play("unit_left");
        return;
    }

    //referenced in move, takes in difference between start and endPos
    private void MoveAnimation(Vector2 diff) {
        if (diff == Vector2.up) {
            animator.Play("unit_up");
            return;
        }
        if (diff == Vector2.down) {
            animator.Play("unit_down");
            return;
        }
        if (diff == Vector2.left) {
            animator.Play("unit_left");
            return;
        }
        if (diff == Vector2.right) {
            animator.Play("unit_right");
            return;
        }
    }
